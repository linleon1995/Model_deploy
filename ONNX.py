...
def torch_to_ONNX(
        ...
        dynamic_axes={'input' : {1: 'channel', 2 : 'depth', 3: 'height', 4: 'width'},
...
def torch_to_ONNX_3d_mask(dummy_input, model, save_filename):
...
def ONNX_inference_mask(inputs, onnx_model):
    ort_session = onnxruntime.InferenceSession(onnx_model)
    # compute ONNX Runtime output prediction
    input_names = ort_session.get_inputs()
    ort_inputs = {
