...
def seg_nrrd_write(filename, voxels, direction, origin, spacing, space):
    direction = direction * np.tile(spacing[:,np.newaxis], (1, 3))
    seg_header = build_nrrd_seg_header(voxels, direction, origin, space=space)
    nrrd.write(filename, voxels, seg_header)
    print('Seg nrrd data conversion completed.')


def raw_nrrd_write(filename, voxels, direction, origin, spacing, space):
    direction = direction * np.tile(spacing[:,np.newaxis], (1, 3))
    nrrd_header = build_nrrd_header(voxels, direction, origin, space=space)
