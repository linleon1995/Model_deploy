...
def preprocess_op(ct_img, spacing, filename):
    import time
    t1 = time.time()
    img = HU2uint8(ct_img)
    # print('HU2uint8', img.max(), img.min(), img.shape)
    ...
    t3 = time.time()
    # print('lung mask', seg_img.max(), seg_img.min(), seg_img.shape)

    # resample
    resampled_img, resampled_spacing = resample(img, spacing, order=3)
    seg_img, resampled_spacing = resample(seg_img, spacing, order=3)
    lung_mask, _ = resample(lung_mask, spacing, order=3)
    lung_mask = np.where(lung_mask>0, 1, 0)
    t4 = time.time()
    # print('resample', seg_img.max(), seg_img.min(), seg_img.shape)
    ...
    print(f'Extract lung mask {t3-t2}')
    print(f'resample {t4-t3}')
